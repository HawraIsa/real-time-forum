...
// HandleLike handles the like or dislike action for a post
func HandleLike(w http.ResponseWriter, r *http.Request) {
	...
	if userName != "" {
		postId, _ := strconv.Atoi(r.URL.Query().Get("postID")) // Get the post ID from query parameters and convert to integer
		...
		likes, dislikes, actionTaken, err := setPostLikes(postId, userID, isDislike) // Call function to set post likes or dislikes
		if err != nil {                                                              // If there is an error setting post likes or dislikes
		...
		http.Error(w, "You must be logged in to like or dislike a post", http.StatusUnauthorized) // Return error if user is not logged in
	}
}

// setPostLikes sets the like or dislike for a post
func setPostLikes(postId int, userId int, isDislike int) (int, int, int, error) {
	var id int                                                                                                                                     // Variable to hold the postLikeId
	var isDislikeDB int                                                                                                                            // Variable to hold the current dislike status from the database
	err := DB.QueryRow("SELECT postLikeId, postIsDisliked FROM postLikes WHERE postId = ? and userId = ?", postId, userId).Scan(&id, &isDislikeDB) // Check if there is a like or dislike from before
	if err == sql.ErrNoRows {                                                                                                                      // If no likes or dislikes exist
		err = DB.QueryRow("Insert into postLikes (postIsDisliked, postId, userId) values (?, ?, ?)", isDislike, postId, userId).Scan() // Add a new like or dislike to the database
		if err != nil && err != sql.ErrNoRows {                                                                                        // If there is an error inserting the like or dislike
			return 0, 0, 0, err // Return error
		}
		likes, dislikes, _, _ := getPostLikes(postId) // Get the updated likes and dislikes for the post
		...
		if (isDislikeDB == 1 && isDislike == 1) || (isDislikeDB == 0 && isDislike == 0) { // If the same action is being repeated, delete the like or dislike
			err = DB.QueryRow("delete from postLikes where postLikeId = ?", id).Scan() // Delete the like or dislike
			...
			err = DB.QueryRow("update postLikes set postIsDisliked = ? where postLikeId = ?", isDislike, id).Scan() // Update the like or dislike
